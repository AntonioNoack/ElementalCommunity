...
class MatchingLayout(ctx: Context, attributeSet: AttributeSet?) : FrameLayout(ctx, attributeSet) {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val maxSizeX = when (MeasureSpec.getMode(widthMeasureSpec)) {
            MeasureSpec.EXACTLY -> max(
                0,
                MeasureSpec.getSize(widthMeasureSpec) - (paddingLeft + paddingRight + marginLeft + marginRight)
            )
            MeasureSpec.AT_MOST -> max(
                0,
                MeasureSpec.getSize(widthMeasureSpec) - (paddingLeft + paddingRight + marginLeft + marginRight)
            )
        ...
        val maxSizeY = when (MeasureSpec.getMode(heightMeasureSpec)) {
            MeasureSpec.EXACTLY -> max(
                0,
                MeasureSpec.getSize(heightMeasureSpec) - (paddingTop + paddingBottom + marginTop + marginBottom)
            )
            MeasureSpec.AT_MOST -> max(
                0,
                MeasureSpec.getSize(heightMeasureSpec) - (paddingTop + paddingBottom + marginTop + marginBottom)
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        children.firstOrNull()
            ?.layout(childLeft, childTop, childLeft + realWidth, childTop + realHeight)
        // throw RuntimeException("$childLeft, $childTop, $right, $bottom, $realWidth, $realHeight")
    }

    private fun View.marginLeft() = (this.layoutParams as? MarginLayoutParams)?.leftMargin ?: 0
    private fun View.marginTop() = (this.layoutParams as? MarginLayoutParams)?.topMargin ?: 0
    private fun View.marginRight() = (this.layoutParams as? MarginLayoutParams)?.rightMargin ?: 0
    private fun View.marginBottom() = (this.layoutParams as? MarginLayoutParams)?.bottomMargin ?: 0
